...
export const transfer = async (sender_id: number, recipient_id: string, amount: number): Promise<TransactionOutput> => {
  ...
  if (checkSenderExists.success === false || checkRecipientExists.success === false) {
    return {
      success: false,
  ...
  const checkRecipientIsBen = await checkUserIsBeneficiary(sender_id, String(checkRecipientExists.data.email));
   
  if (!checkRecipientIsBen) {
    return {
      success: false,
      message: "Recipient must be your beneficiary before you can transfer money",
      data: {}
    }
  }
